...
*   **Producción Energía**: Basado en fórmula con Technology Service (ver BuildingsService.cs:57-62).
*   **Consumo Deuterio**: 10 * Nivel * 1.1^Nivel.
*   **Nota de Implementación**: El cálculo exacto depende de la tecnología Energy, pero no está implementado completamente en el código actual (simplificado sin Technology Service).
...
*   **Requisito**: Robotics Factory Nivel 10 (pendiente: Computer Technology no implementado en validación).
*   **Efecto**: En teoría reduce a la mitad el tiempo por nivel, pero NO está implementada en BuildingService.cs (ver nota abajo).
*   **Factor Coste**: 2.0
*   **Nota de Implementación**: La lógica para reducir tiempos de construcción con Nanite Factory no existe actualmente en BuildingService.cs.
...
*   **Coste Base**: Metal: 0, Cristal: 50,000, Deuterio: 100,000 (pendiente: energía no implementada en costos).
*   **Efecto**: +5 Campos por nivel (Ocupa 1, neto +4) (pendiente: no implementado en código).
*   **Requisito**: Nanite Factory Nivel 1, Energy Technology Nivel 12 (pendiente: no validado en código).
*   **Factor Coste**: 2.0
*   **Nota de Implementación**: El cálculo de campos adicionales y validaciones no está implementado actualmente.
...
### Nanite Factory (Fábrica de Nanobots) - Detalles de Implementación
*   **Producción**: El código actual NO implementa la reducción del 50% por nivel para construction speed.
*   **Requisitos**: Solo verifica Robotics Factory Nivel 10. Computer Technology no está validado.

### Fusion Reactor - Detalles de Implementación  
*   **Consumo Deuterio**: Implementado en producción, pero el cálculo exacto con Technology Service no está aplicado.
